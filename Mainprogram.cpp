...
string rootname;
vector<vector<string>> files_list;  //For storing list of files
void onend(string);
void onend(void);
void onend(int);
void Raw_off(void);
...
stack<string> backstack,forwardstack;//stack to store the forwrd and backward activity
...
//to get the cursor position in the terminal
//through the use of escape sequence 
...
//Flooding vector with the Details of file to present 
		...
		dirent *current_read;// dirent.h header helps to open and close directory
		char path[]=".";
		dir = opendir(path);//opens all directory 
		if(dir == NULL)
		{
		  onend("No directory Found! Is empty");
		}
		...
		while((current_read=readdir(dir)) != NULL)
		{
		  ...
		  vector<string>temp; 
		      ...
		      */
		      ...
		      //userid grid and permissions all pushed to the vector and into filedetails
		...
		closedir(dir);
		//sorting is done based on the filename first charcter 
